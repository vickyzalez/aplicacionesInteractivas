...
import controladores.SistemaGimnasio;
...
//              Validaci�n sobre campos ingresados
				while (vl_ok == 0){
					try {
						vl_dni = Integer.parseInt(textField.getText());
						vl_ok = 1;
					} catch (NumberFormatException e1) {
						JOptionPane.showMessageDialog(null, "Ingrese solo n�meros en el dni");
						vl_ok = 0;
					}
					...
					try {
						...
						JOptionPane.showMessageDialog(null, "Ingrese solo n�meros en el tel�fono");
						vl_ok = 0;
					}
				}

				SocioView s = new SocioView( vl_dni , textField_1.getText(), 
						             textField_2.getText(), textField_3.getText(), 
						             vl_telefono , textField_5.getText()); 
				SistemaGimnasio sg  = SistemaGimnasio.getGimnasio();
				sg.agregarSocio(s);
				dispose();
			}
		});
